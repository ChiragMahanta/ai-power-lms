...
import { useLoggedOut, useGetUserHook } from '@/hooks/User.hook'
...
const Navbar = () => {
  ...
  const { data: fetchedUser } = useGetUserHook()
  ...
  const userInitial = displayUser?.fullName?.charAt(0)?.toUpperCase() || 'U'
      ...
      <div>
        <Popover>
          <PopoverTrigger className='cursor-pointer flex items-center gap-3 hover:bg-gray-50 px-3 py-1.5 rounded-full transition-colors'>
            <Avatar>
              <AvatarImage src={displayUser?.profilePhoto || ''} alt={displayUser?.fullName || 'User'} />
              <AvatarFallback className="bg-blue-600 text-white font-bold">
                {userInitial}
              </AvatarFallback>
            </Avatar>
            <span className='font-medium text-gray-700 hidden md:inline'>
              {displayUser?.fullName || 'Guest'}
            </span>
          </PopoverTrigger>
